...
parser.add_argument('--choose_model', type=str, required=True, default='LSTM',
                    help='model name, options: [LSTM, BiLSTM, CNN+LSTM,CNN]')
...
def get_train_data(data):
    ...
    for i in range(len(data)):
        ...
        for j in range(0,18):
            for i in range(len(normalized_data) - 150):# 预测15s后
...
with strategy.scope():
    ...
    if choose_model == 'LSTM':    
        ...
        model = Sequential()
        model.add(Bidirectional(LSTM(20,return_sequences=True),input_shape=(100,2)))
        model.add(Dropout(0.25))
        model.add(Bidirectional(LSTM(10)))
        model.add(Dropout(0.25))
        model.add(Dense(2))

    elif choose_model=='CNN+LSTM':
        model = Sequential()
        ...
        model = Sequential()
        model.add(Conv1D(filters=32, kernel_size=3, padding="same",activation="relu",input_shape=(100,2)))
        model.add(MaxPooling1D(pool_size=2))
        model.add(Flatten())
        model.add(Dense(32, activation="relu"))  # 可选的全连接层，增加模型表达力
        model.add(Dense(2))
    
    model.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
    history = model.fit(x_train, y_train,batch_size = 200,epochs=15,shuffle=True,use_multiprocessing=True,workers=4)
#history = model.fit(x_train, y_train,batch_size = 150,epochs=15)
...
plt.plot(range(len(loss)), loss, 'red', label='train loss',linewidth=1.5)
plt.xlabel("Epoch")
plt.ylabel("loss")
plt.grid(linestyle='-.',alpha=0.3)
plt.legend()
check_path = os.path.join(folder_path, choose_model,"img","loss.png")
dir_path = os.path.dirname(check_path)
if not os.path.exists(dir_path):
    os.makedirs(dir_path, exist_ok=True)
# 检查路径的父目录是否存在，如果不存在则创建它
...
if not os.path.exists(dir_path):
    os.makedirs(dir_path, exist_ok=True)
plt.savefig(folder_path + choose_model + "/img/" + "loss.png")
...
for i in range(len(test_data)):
    ...
    y_pre_all = model.predict(x_test,batch_size=1)
    ...
    data_rows = [{'time': t, 'Real': r, 'Predicted Value': p} for t, r, p in zip(time[-len(preds):], trues, preds)]
    print("############################################################################")
    # 将字典列表转换为DataFrame
    df = pd.DataFrame(data_rows)
    # 将DataFrame保存到CSV文件
    df.to_csv(folder_path + choose_model +'/senior1.csv', index=False)

    data_rows = [{'time': t, 'Real': r, 'Predicted Value': p} for t, r, p in zip(time[-len(preds):], trues_t, preds_t)]
    # 将字典列表转换为DataFrame
    df = pd.DataFrame(data_rows)
    # 将DataFrame保存到CSV文件
...
#         plt.text(xx, yy, formatted_text, fontsize=13,ha='left', va='top')
#         plt.savefig(folder_path + choose_model + "/img/" + str(dict[i]) + 'ceiling' + ".png")
